...
#include "led_rse/vdrse.h"

void nics::VdrseNodelet::camera_callback(
...
Sophus::SE3d nics::VdrseNodelet::posemsg_to_SE3(const geometry_msgs::PoseStamped pose)
{
    return Sophus::SE3d(
        Eigen::Quaterniond(
...
geometry_msgs::PoseStamped nics::VdrseNodelet::SE3_to_posemsg(
...
nav_msgs::Odometry nics::VdrseNodelet::SE3_to_odommsg(
    const Sophus::SE3d pose_on_SE3,
...
void nics::VdrseNodelet::ugv_pose_callback(const geometry_msgs::PoseStamped::ConstPtr& pose)
{
...
void nics::VdrseNodelet::uav_pose_callback(const geometry_msgs::PoseStamped::ConstPtr& pose)
{
...
void nics::VdrseNodelet::uav_setpt_callback(const geometry_msgs::PoseStamped::ConstPtr& pose)
{
    uav_stpt_msg = *pose;
}

void nics::VdrseNodelet::map_SE3_to_publish(
...
void nics::VdrseNodelet::solve_pose_w_LED(cv::Mat& frame, cv::Mat depth)
{    
    if(!LED_tracker_initiated_or_tracked)        
    {
...
void nics::VdrseNodelet::apiKF(int DOKF)
{
    switch (DOKF)
    {
    case kfINITIATE:
...
void nics::VdrseNodelet::recursive_filtering(cv::Mat& frame, cv::Mat depth)
{
...
void nics::VdrseNodelet::get_correspondence(
...
std::vector<Eigen::Vector2d> nics::VdrseNodelet::shift2D(
...
void nics::VdrseNodelet::solve_pnp_initial_pose(std::vector<Eigen::Vector2d> pts_2d, std::vector<Eigen::Vector3d> pts_3d)
{
    Eigen::Matrix3d R;
    Eigen::Vector3d t;
...
std::vector<Eigen::Vector2d> nics::VdrseNodelet::LED_extract_POI(cv::Mat& frame, cv::Mat depth)
{   
...
std::vector<Eigen::Vector2d> nics::VdrseNodelet::LED_extract_POI_alter(cv::Mat& frame, cv::Mat depth)
{   
    std::vector<Eigen::Vector2d> pts_2d_detected;
...
std::vector<Eigen::Vector3d> nics::VdrseNodelet::pointcloud_generate(std::vector<Eigen::Vector2d> pts_2d_detected, cv::Mat depthimage)
{
...
bool nics::VdrseNodelet::initialization(cv::Mat& frame, cv::Mat depth)
{
...
void nics::VdrseNodelet::correspondence_search_2D2DCompare(
    std::vector<Eigen::Vector2d>& pts_2d_detected,
...
inline double nics::VdrseNodelet::calculate_MAD(std::vector<double> norm_of_points)
{
...
void nics::VdrseNodelet::reject_outlier(std::vector<Eigen::Vector2d>& pts_2d_detect, cv::Mat depth)
{
...
void nics::VdrseNodelet::set_image_to_publish(double freq, const sensor_msgs::CompressedImageConstPtr & rgbmsg)
{    
    char hz[40];
    char fps[10] = " fps";
    sprintf(hz, "%.2f", freq);
...
void nics::VdrseNodelet::log(double ms)
{
    led_rse::vdrse_log logdata_entry_led;
    ...
    led_rse::vdrse_log logdata_entry_uav;
...
void nics::VdrseNodelet::terminal_msg_display(double hz)
{
...
void nics::VdrseNodelet::record_ms()
{
    std::ofstream save(
